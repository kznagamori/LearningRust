...
Rustで平衡二分木構造、特に `AVL` 木を実装するための完全なサンプルプログラムを提供します。このプログラムでは基本的な `AVL` 木の挿入と単純な回転操作を含んでいます。`AVL` 木は、ノードに高さ情報を保持し、挿入や削除の際に左右の部分木の高さの差が特定の範囲を超えないように保ちます。
...
このコードでは、`TreeNode` 構造体でツリーの各ノードを定義し、各ノードには値、高さ、左右の子ノードへの参照が含まれます。`AVLTree` 構造体では、木の根を管理し、新しい要素の挿入や中間順での巡回を行います。挿入時には、バランスファクター（左右の部分木の高さ差）を計算し、必要に応じて左回転または右回転を行って木を再バランスします。

これは基本的な `AVL` 木の実装であり、より複雑な操作（削除、複数回転など）を加えることで完全な `AVL` 木を構築できます。
